...
<div class="m-content">
	...
	<div class="m-portlet m-portlet--mobile">
        ...
        {{ Form::model($record, array('route' => ['superadmin.schedule.update', $record->id], 'id'=>'edit-schedule-form', 'class' => 'm-form')) }}
		  @method('PUT')
				...
				<div class="form-group m-form__group row">
					...
					<div class="col-lg-6">
						<select class="form-control" name="user_id">
							...
							<option {{($record->user_id == $value->id) ? 'selected' : '' }} value="{{$value->id}}">{{$value->name}}</option>
							@endforeach
						    @endif
						</select>
						@error('user_id')
...
@endsection
@section('scripts')
{!! JsValidator::formRequest('App\Http\Requests\Backend\PowerScheduleRequest', '#edit-schedule-form'); !!}
